      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <ScrollReveal variant="fade-up" delay={150}>
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
            ...
            {/* Left Column: AI Robot Inspector Showcase Card */}
            <div className="lg:col-span-5 text-left">
              <div className="relative rounded-3xl overflow-hidden border border-indigo-100 bg-gradient-to-b from-indigo-50/50 to-white p-4 sm:p-5 shadow-xl shadow-indigo-100/50 group h-full flex flex-col justify-between">
            ...
            {/* Right Column: ATS Sandbox Screen & Core Matrices directly facing the image */}
            <div className="lg:col-span-7 space-y-4">
              <div className="bg-zinc-100/70 border border-zinc-200/60 p-6 rounded-3xl shadow-xl shadow-zinc-200/10 relative overflow-hidden group">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-12 gap-6 items-stretch">
                  ...
                  <div className="sm:col-span-6 bg-white border border-zinc-200 rounded-2xl p-4 relative overflow-hidden min-h-[300px] flex flex-col justify-between shadow-md">
                    ...
                    <button
                      disabled={isScanning}
                      onClick={handleScan}
                      className="w-full mt-3 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 text-white font-bold text-xs flex items-center justify-center gap-1.5 shadow-md shadow-indigo-600/20 transition-all cursor-pointer disabled:opacity-50"
                    >
                      {isScanning ? (
                        <>
                          <RefreshCw className="w-3.5 h-3.5 animate-spin" /> Scanning Keywords...
                        </>
                      ) : (
                        <>
                          <Play className="w-3.5 h-3.5 fill-current" /> Run ATS Audit
                        </>
                      )}
                    </button>
                  </div>

                  {/* ATS Parser Diagnostics Terminal Side */}
                  <div className="sm:col-span-6 bg-white border border-zinc-200 rounded-2xl p-4 flex flex-col justify-between shadow-md text-left">
                    <div className="flex items-center justify-between border-b border-zinc-100 pb-3">
                      <div className="space-y-0.5">
                        <span className="text-[8px] font-black text-indigo-600 tracking-wider uppercase font-display">Target Profile</span>
                        <h4 className="text-xs font-black text-zinc-800 tracking-tight font-display">Staff Software Engineer</h4>
                      </div>
                      <span className="text-[8px] font-bold text-zinc-400 bg-zinc-50 border border-zinc-200 px-1.5 py-0.5 rounded-sm">
                        JD SCAN V2
                      </span>
                    </div>
              ...
              {/* Core Keyword & Layout Matrices Bar - directly opposite the image */}
              <div className="bg-white border border-zinc-200/80 p-4 sm:p-4.5 rounded-2xl shadow-md shadow-zinc-100 text-left space-y-2">
                <h3 className="text-xs font-black text-indigo-600 uppercase tracking-wider font-display">
                  Core Keyword & Layout Matrices
                </h3>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 text-[11px] font-semibold text-zinc-600 font-display">
                  <div className="flex items-center gap-1.5 text-zinc-700">
                    <Check className="w-3.5 h-3.5 text-indigo-600 shrink-0 stroke-[3]" /> Keyword density
                  </div>
                  <div className="flex items-center gap-1.5 text-zinc-700">
