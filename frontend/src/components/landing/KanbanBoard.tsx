...
export default function KanbanBoard() {
  ...
  const getPriorityColor = (prio: string) => {
    switch (prio) {
      ...
      case "Medium": return "bg-amber-500/10 text-amber-450 border-amber-500/20";
      default: return "bg-zinc-950 text-zinc-500 border-zinc-850";
    }
  };
  ...
  const getLogoColor = (logo: string) => {
    switch (logo) {
      ...
      case "V": return "bg-black text-white border border-zinc-800";
  ...
  return (
    <section id="job-tracker-kanban" className="py-24 bg-[#06060c] relative z-10 border-t border-zinc-900">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center mb-16">
          <div className="lg:col-span-5 text-left order-last lg:order-first">
            ...
            <div className="bg-zinc-900/60 border border-zinc-850 p-4 rounded-2xl flex items-center gap-3 shadow-sm">
          ...
          <div className="lg:col-span-7 bg-zinc-950/40 border border-zinc-850 p-5 rounded-3xl overflow-x-auto shadow-[inset_0_2px_20px_rgba(0,0,0,0.5)] min-w-full lg:min-w-0">
                  ...
                  <div key={col.id} className="flex flex-col gap-3.5">
                    ...
                    <div className="flex justify-between items-center px-1">
                      <div className="flex items-center gap-1.5">
                        ...
                        <h4 className="text-xs font-black text-zinc-300 font-display">{col.label}</h4>
                      </div>
                      <span className="text-[10px] font-black text-zinc-300 bg-zinc-950 border border-zinc-850 px-1.5 py-0.5 rounded-sm">
                        {colCards.length}
                      </span>
                    </div>

                    {/* Cards Container */}
                    <div className="bg-zinc-950/50 rounded-2xl p-2.5 flex-1 min-h-[220px] flex flex-col gap-2.5">
                      {colCards.map((card) => (
                        <div
                          key={card.id}
                          onClick={() => moveCard(card.id)}
                          className="bg-zinc-900 border border-zinc-850 hover:border-indigo-500/40 p-3.5 rounded-xl text-left shadow-md cursor-pointer hover:bg-zinc-900/80 transition-all duration-300 transform hover:-translate-y-0.5 select-none relative group"
                        >
                          <div className="flex justify-between items-start gap-2 mb-2">
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                <h5 className="text-[10px] font-black text-white leading-none font-display">{card.company}</h5>
                                <p className="text-[8px] text-zinc-400 font-semibold mt-0.5">{card.role}</p>
                              </div>
                            </div>
                          ...
                          <div className="flex justify-between items-center text-[8px] font-semibold text-zinc-500 pt-2 border-t border-zinc-850">
                            <span className="text-zinc-300 font-extrabold">{card.salary}</span>
                            <span className="flex items-center gap-0.5 text-zinc-400">
                              <Calendar className="w-2.5 h-2.5 text-zinc-500" /> {card.date}
                            </span>
                          </div>
                      ...
                      <button className="py-2 border border-dashed border-zinc-800 hover:border-indigo-500/40 rounded-xl text-zinc-400 hover:text-white flex items-center justify-center gap-1 text-[10px] font-bold transition-all bg-zinc-900/40 hover:bg-zinc-900/60 cursor-pointer">
